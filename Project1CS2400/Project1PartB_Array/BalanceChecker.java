...
public class BalanceChecker {
	...
	public static boolean checkBalance(String expression) {
		ArrayImplementation<Character> my = new ArrayImplementation<>();
		int index = 0;
		...
		while ((isBalanced == true && index < expression.length())) {
			...
			case '}':
				if (my.isEmpty()) // if my stack is empty then isBalanced is false
					...
					if (isPaired(open, expression.charAt(index))) // if open aka top of stack is paried with the
																	// expression at the char index given
	...
	public static String convertToPostFix(String infix) {
		...
		// for some reason my methods doubles all the operators so i am doing string
		// replacement
	...
	public static void captureOp(Pres firstOp, ArrayImplementation my, StringBuilder postFix) {
		while (!my.isEmpty()) {
	...
	static VectorImplementation<String> preFix = new VectorImplementation<>(); 

	static int global = 0;

	public static String postFixToPreFix(String postFix, char h) {
		// on the array implementation this method does not work so I am using the Vector for this method
